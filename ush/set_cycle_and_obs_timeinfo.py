...
def get_obs_retrieve_times_by_day(
    ...
    # Get the list of forecast fields to be verified from the verification
    # portion of the SRW configuration file.
    vx_fields = vx_config['VX_FIELDS']

    # Define a list of dictionaries containing information about all the obs
    # types that can possibly be used for verification in the SRW.  Each
    ...
    # or instantaneous fields), and a list of the forecast fields that the
    # obs type can be used to verify.
    all_obs_info \
    = [{'obtype': 'CCPA',   'time_type': 'cumul', 'fcst_fields': ['APCP']},
       {'obtype': 'NOHRSC', 'time_type': 'cumul', 'fcst_fields': ['ASNOW']},
       {'obtype': 'MRMS',   'time_type': 'inst',  'fcst_fields': ['REFC', 'RETOP']},
       {'obtype': 'NDAS',   'time_type': 'inst',  'fcst_fields': ['ADPSFC', 'ADPUPA']}
      ]

    # Create new list that has the same form as the list of dictionaries
    # defined above but contains only those obs types and forecast fields
    # that will be needed in the verification.
    obs_info = []
    for obs_dict in all_obs_info.copy():
        obtype = obs_dict['obtype']
        fcst_fields = obs_dict['fcst_fields']
        fcst_fields = [field for field in fcst_fields if field in vx_fields]
        obs_dict = obs_dict.copy()
        obs_dict['fcst_fields'] = fcst_fields
        if fcst_fields: obs_info.append(obs_dict)
    ...
    # value needs to be build by adding values from previous times.  For
    ...
    for obs_dict in obs_info:
        ...
        # Consider all forecast fields to be verified for the current obs type.
        for field_fcst in fcst_fields:

            # Get the list of accumulation intervals for the current cumulative obs
            # type and forecast field combination.
            accum_intvls_array_name = "".join(["VX_", field_fcst, "_ACCUMS_HRS"])
            accum_intvls_hrs = vx_config[accum_intvls_array_name]

            for cycle_start_time in cycle_start_times:

                # Loop through the accumulation intervals for this obs type and forecast
                # field combination.
                for accum_intvl_hrs in accum_intvls_hrs:
                    ...
                    # compared to the corresponding cumulative forecast field (for the current
