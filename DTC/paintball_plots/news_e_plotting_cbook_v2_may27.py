...
### Qualitative colors (bright):
   ...
   paintball_colors = matplotlib.colors.ListedColormap([q1, q2, q3, q4, q5, q6, q7, q8, q9, q10, q11, q12, b8, purple3, red3, green3, blue3, orange3])
...
def mymap(sw_lat, sw_lon, ne_lat, ne_lon, lat_1, lat_2, lat_0, lon_0, damage_files, resolution='i', area_thresh = 1000., counties='False'):
...
#######################
   ...
   fig = P.figure(figsize=(6.5,8))

   ax1 = fig.add_axes([0.01,0.1,0.98,0.85])
...
##### handle axes variations between paintball, object matching, and regular plots: #######
   ...
   if (verif == 'True'):   
      ...
      ax2 = fig.add_axes([0.05,0.06,0.43,0.02])
      ax3 = fig.add_axes([0.52,0.06,0.43,0.02])
#      ax2 = fig.add_axes([0.04,0.03,0.44,0.05])   
#      ax3 = fig.add_axes([0.54,0.03,0.44,0.05])
   elif (object == 'True'):   
      map = mymap(sw_lat, sw_lon, ne_lat, ne_lon, lat_1, lat_2, lat_0, lon_0, damage_files, resolution, area_thresh, counties='False') 
      ...
      ax2 = fig.add_axes([0.04,0.06,0.92,0.02])   
      ax3 = fig.add_axes([0.96,0.03,0.02,0.05])
...
def env_plot(map, fig, ax1, ax2, ax3, x, y, plot, var1, var2, radmask, t, init_label, valid_label, domain, outdir, q_u, q_v, scale, t_star, t_plus, spec, quiv, showmax='False'):
...
############# Create filled contour plot, with intervals highlighted for var1 data:    #################
   ...
   pr = map.contourf(x, y, radmask, colors=[cb_colors.gray2, cb_colors.gray2], levels=[0.1, 1000.], alpha=0.8)
...
def paintqc_plot(map, fig, ax1, ax2, ax3, x, y, vx, vy, plot, var1, var2, radmask, ob_count, member_count, t, init_label, valid_label, domain, outdir, t_star, t_plus):
...
############ Add text labels of plot titles and initialization and valid forecast time: ###############
   ...
   #t1 = fig.text(0.05, 0.95, plot.var1_title, fontsize=14, fontweight='bold', color=plot.var1_tcolor)
   #t2 = fig.text(0.05, 0.92, plot.var2_title, fontsize=14, fontweight='bold', color=plot.var2_tcolor)
   #t3 = fig.text(0.20, 0.92, init_label, fontsize=12)
   #t4 = fig.text(0.76, 0.92, valid_label, fontsize=12)

   ob_label = 'Ob Gridpoints:              ' + str(ob_count)
   member_label = 'Avg. Mem. Gridpoints:  ' + str(member_count)

#   t6 = fig.text(0.2, 0.04, ob_label, backgroundcolor=cb_colors.gray2, fontsize=12, fontweight='bold', color=plot.var1_tcolor)
#   t7 = fig.text(0.2, 0.01, member_label, backgroundcolor=cb_colors.gray2, fontsize=12, fontweight='bold', color=plot.var2_tcolor)
...
############ shade verification objects in dark gray.  If forecast objects are not present, add label.  ##############
   if (np.max(var2) > 0.): 
##   p3 = map.contourf(vx, vy, radmask, colors=[cb_colors.gray5, cb_colors.gray5], levels=[0.1, 1000.], alpha=0.6)
      proxy = []
      ...
      for n in range(0, var2.shape[0]):
        #colorpull = pc.get_facecolor()[0] for pc in 
        p2 = map.contourf(x, y, var2[n,:,:], colors=[plot.var2_colors[n]], levels=plot.var2_levels, alpha=.65, extend=plot.extend)
        p22 = map.contour(x, y, var2[n,:,:], colors=[plot.var2_colors[n]], levels=plot.var2_levels, alpha=.9, extend=plot.extend, linewidths=.3)
        proxy += [matplotlib.pyplot.Rectangle((0,0),1,1,color = plot.var2_colors[n])]
          # for pc in p2.collections
                 
        #print (get_facecolor())
       #[plot.var2_colors[n], plot.var2_colors[n]] 
         #legend=fig.legend(['First line', 'Second line'])

      #sigma = 3.5 # how much do we want to smooth for observation contours
      #var1 = gaussian_filter(var1, sigma)

#      p1 = map.contourf(vx, vy, var1, colors=plot.var1_colors, levels=plot.var1_levels, alpha=0.0, extend=plot.extend)
      #p12 = map.contour(vx, vy, var1, colors=plot.var1_colors, levels=plot.var1_levels, alpha=1, extend=plot.extend, linewidths=.5)
      #p12 = map.contour(vx, vy, var1, colors=plot.var1_colors, levels=[9, 27, 37.5], alpha=1, extend=plot.extend, linewidths=[ 1.1, 1.6, 2.0])
      p12 = map.contour(vx, vy, var1, colors=plot.var1_colors, levels=[30], alpha=1, extend=plot.extend, linewidths=[1.1])      

      ###Add the contour line labels
      fmt = {}
      #strs = ['10', '30', '40']
      strs = ['30']
      for l, s in zip(p12.levels, strs):
        fmt[l] = s
      matplotlib.pyplot.clabel(p12, p12.levels, inline=True, fmt=fmt, fontsize=10, inline_spacing=1)
      
      proxy += [matplotlib.pyplot.Rectangle((0,0),1,.5,color = plot.var1_colors[0])]
      matplotlib.pyplot.legend(proxy, ["mem1(ctrl)", "mem2", "mem3", "mem4","mem5","mem6","mem7","mem8","mem9","MRMS"], loc ="upper left")
      #matplotlib.pyplot.legend(proxy, ["mem1(ctrl)", "mem2", "mem3", "mem4","mem5","mem6","mem7","mem8","mem9","MRMS"], loc ="lower left")
      ###LEGEND###
      
      #matplotlib.pyplot.legend(["1", "2", "3","4"], loc ="lower right")
   else:
...
#         coll.remove()
      ...
      #for coll in p3.collections:
      #   coll.remove()
      for coll in p12.collections:
         coll.remove()
      for coll in p22.collections:
         coll.remove()
   else:
      t5.remove()
...
############# Create filled contour plot, with intervals highlighted for var1 data:    #################
   ...
   p1 = map.contourf(x, y, var1, cmap=plot.cmap, levels = plot.var1_levels, alpha=plot.alpha, extend='both')
#   p2 = map.contour(x, y, var1, colors='k', levels = plot.var1_levels, linewidths=0.4, alpha=0.25, linestyles='solid')
...
############ Scatter OK mesonet data: #################
   ...
   p5 = map.scatter(ob_x, ob_y, c=ob_var, s=60, linewidth=0.5, edgecolor=cb_colors.gray5, cmap=ob_cmap, alpha=0.8, vmin=np.min(ob_levels), vmax=np.max(ob_levels))
   p5.cmap.set_over(cb_colors.red7)
   p5.cmap.set_under(cb_colors.blue7)
