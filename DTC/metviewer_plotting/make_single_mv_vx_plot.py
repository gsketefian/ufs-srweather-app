...
def parse_args(argv, valid_vx_plot_params):
    ...
    parser.add_argument('--mv_host',
                        type=str,
                        required=True,
                        help='Host on which METviewer is installed.')
...
def generate_metviewer_xml(cla, valid_vx_plot_params, mv_databases_dict):
    ...
    -------
    [unnamed]:
      Path to yaml METviewer host configuration file.
    ...
    mv_host_config_fp = Path(os.path.join(cla.mv_host_config_fp)).resolve()
    mv_host_config = load_config_file(mv_host_config_fp)

    all_hosts = sorted(list(mv_host_config.keys()))
    if cla.mv_host not in all_hosts:
        msg = dedent(f"""
            The host specified on the command line (cla.mv_host) does not have a
            corresponding entry in the METviewer host configuration file
            (mv_host_config_fp):
              cla.mv_host = {get_pprint_str(cla.mv_host)}
              mv_host_config_fp = {get_pprint_str(mv_host_config_fp)}
    ...
    mv_host_config_dict = mv_host_config[cla.mv_host]
    ...
    for model_name_short in cla.model_names_short:
        if model_name_short not in model_names_short_avail_in_db:
            msg = dedent(f"""
                ...
                database configuration file (cla.mv_database_config_fp)
                  cla.mv_database_config_fp = {get_pprint_str(cla.mv_database_config_fp)}
                  cla.mv_database = {get_pprint_str(cla.mv_database)}
    ...
    jinja2_vars = {"mv_host": cla.mv_host,
                   "mv_host_config_dict": mv_host_config_dict,
    ...
    return(mv_host_config_dict['mv_batch_fp'], output_xml_fp)
...
def make_single_mv_vx_plot(argv):
    ...
    msg = dedent(f"""
        Obtaining METviewer database information from file
          {get_pprint_str(cla.mv_database_config_fp)}
        ...
        """)
    logging.info(msg)
